...
y_train = read.table("data/train/y_train.txt", col.names=c("activity_id"))  # max = 6
...
features = read.table("data/features.txt", col.names=c("feature_id", "feature_label"),)  #561
...
activity_labels = read.table("data/activity_labels.txt", col.names=c("activity_id", "activity_label"),) #
...
# write data to file
write.table(x=aggdata, file="result_dataset.txt", row.names = FALSE)